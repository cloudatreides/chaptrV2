...
import { ArrowRight, Plus, Pencil, Sparkles } from 'lucide-react'
        ...
        {hasCharacters && (
          ...
          >
            {[
              ...
              { val: UNIVERSES.filter((u) => !u.locked).length.toString(), label: 'Universes', color: '#c84b9e' },
            ].map((s) => (
              <div key={s.label} className="flex-1 rounded-xl p-3 flex flex-col gap-1" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <p className="text-white/30 text-[11px] font-medium">{s.label}</p>
              </div>
            ))}
          </motion.div>
        )}
            ...
            <div className="flex gap-8">
              ...
              <div className="flex-1 flex flex-col gap-8">
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                  <p className="text-accent/50 text-[10px] font-semibold tracking-[2px] uppercase mb-3">Your Universes</p>
