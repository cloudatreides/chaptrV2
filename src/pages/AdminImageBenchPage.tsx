...
const IMAGE_GEN_AUDIT: ImageGenAction[] = [
  ...
  },
  {
    ...
    model: 'Nano Banana (gemini-2.5-flash-image)',
    references: 'twin + companion',
    promptTemplate: 'Randomized at runtime across 5 implied-sex framings: (1) Faces mid-moment — extreme close-up of both faces, eyes closed in bliss, foreheads pressed, sheet covers bottom of frame; (2) Hands gripping sheets — close-up of hands, no faces, knuckles white on bunched sheets; (3) Silhouette through curtain — backlit shapes against a sheer window, no detail; (4) Morning after — peaceful tangled-sheets aftermath; (5) Companion above looking up — POV camera looks up at the companion\'s face, sheet over their shoulders. Each variant uses tight crops or framing devices as the structural safety guardrail — there is literally no body to render below the cropped/covered area.',
    samplePrompts: [
    ...
    notes: 'PROPOSAL — not in prod yet. Bench all 5 variants × 6 models to verify each framing renders cleanly before deciding which ship to the prod pool. Each variant uses a different structural safety guardrail: (1) tight face crop with sheet at bottom; (2) hands-only frame; (3) backlit silhouette with no detail; (4) post-coital aftermath with sheet to collarbones; (5) POV with sheet over shoulders. The bralette/clothing approach used in Undress isn\'t needed here because the FRAMING does the work — there\'s literally no body in the frame to render incorrectly. Variant 3 (silhouette) should be the safest universal pass; variant 1 (faces mid-moment) is the most expressive; variant 4 (morning after) is essentially a spicier Cuddle. If 2.5 refuses any variant, swap that one out before locking the prod pool. Once decided, will wire up via runWindDownAction with kind="makeLove", gem cost 200, affinity bump +12, slotting above Undress in the chat-action panel.',
  },
  {
