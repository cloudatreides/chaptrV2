...
const IMAGE_GEN_AUDIT: ImageGenAction[] = [
  ...
  },
  {
    ...
    model: 'Nano Banana (gemini-2.5-flash-image)',
    references: 'twin + companion',
    promptTemplate: 'Randomized at runtime across 2 action-forward framings: (1) Missionary, his bare back to camera with side cleavage glimpse; (2) Female on top, her bare back to camera with side cleavage glimpse. Both visualize the position in the register of mature visual novels — bare back carries the "in the act" framing, sheets cover everything below the waist/hips, no nipples and no genitals visible by explicit prompt floor.',
    samplePrompts: [
    ...
    notes: 'PROPOSAL — not in prod yet. Bench-only test of the two action-forward framings users actually want. Both are the most likely to soft-refuse on Gemini 2.5 due to "making love" + "bare back" + "side cleavage" hitting safety heuristics — but they\'re the only variants that communicate "in the act" (the implied-only set was rejected as too tame for the lonely-user demographic). Expected outcomes: (a) clean 2.5 render — great, ship it; (b) 2.5 soft-refuses → falls through to FLUX.1 Schnell fallback which will likely render the position cleanly but identity-blind (no twin/companion face match); (c) 2.5 renders a tame Cuddle ignoring the "making love" intent. If 2.5 fails consistently on these, the hybrid play is: route Make love specifically to Schnell, accept identity loss as the tradeoff for getting the visual register right. Once a variant ships cleanly, wire up via runWindDownAction with kind="makeLove", gem cost 200, affinity bump +12, slotting above Undress in the chat-action panel.',
  },
  {
