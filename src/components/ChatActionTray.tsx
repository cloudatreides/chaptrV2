    ...
    <>
      ...
      >
        ...
        {!isOpen && (
          ...
          >
            <Gem size={7} /> {masterMode ? '∞' : gemBalance}
          </span>
        )}
      </button>
            ...
            <div className="flex gap-1 p-2 border-b border-white/5">
              <button
                ...
                style={{
                  ...
                  color: !activeCategory ? '#e060b8' : 'rgba(255,255,255,0.55)',
                }}
              >
                All
              </button>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: activeCategory === cat ? info.color : 'rgba(255,255,255,0.55)',
                    }}
                  >
                    {info.label}
                  </button>
            ...
            <div className="p-2 overflow-y-auto flex-1" style={{ maxHeight: '240px' }}>
              <div className="grid grid-cols-3 gap-1.5">
                ...
                {filteredAvailable.map(action => {
                  ...
                  const cantAfford = !masterMode && action.gemCost > 0 && gemBalance < action.gemCost
                  ...
                  return (
                    <button
                      ...
                      disabled={isDisabled}
                      className="w-full flex flex-col items-center gap-1 p-2.5 rounded-xl transition-all hover:brightness-125 disabled:opacity-50 disabled:hover:brightness-100"
                      style={{
                        background: hoveredAction === action.id ? 'rgba(200,75,158,0.08)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${hoveredAction === action.id ? 'rgba(200,75,158,0.15)' : 'rgba(255,255,255,0.08)'}`,
                      }}
                    >
                      <span className="text-lg">{action.emoji}</span>
                      <span className="text-[10px] text-white/90 font-medium text-center leading-tight">{action.label}</span>
                      <div className="flex items-center gap-1">
                        {action.gemCost > 0 && (
                          <span className="flex items-center gap-0.5 text-[9px]" style={{ color: cantAfford ? '#ef4444' : 'rgba(251,191,36,0.85)' }}>
                      ...
                      {onCooldown && (
                        <span className="text-[8px] text-white/50">used</span>
                      )}
                    </button>
                  )
                })}
                ...
                {filteredLocked.map(action => (
                  ...
                  >
                    ...
                    <span className="text-[8px] text-white/35">{tierLabels[action.minTier]}</span>
                  </div>
                ))}
              </div>
                ...
                {hoveredData ? (
                  ...
                  >
                    <span className="flex items-center gap-1 text-[10px] text-white/50">
                      <Gem size={10} style={{ color: 'rgba(251,191,36,0.7)' }} />
                      {masterMode ? '∞' : gemBalance} gems
                    </span>
